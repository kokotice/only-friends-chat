    ...
    <div className="min-h-screen bg-background flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-md rounded-2xl border border-border bg-card p-8">
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          {mode === "forgot"
            ? resetStep === "email"
              ...
              : "We've sent you the code on your email, please write the code here."
        ...
        {mode === "forgot" ? (
          <>
            {resetStep === "email" ? (
              <form onSubmit={sendResetCode} className="mt-6 space-y-3">
                <div>
              ...
              <form onSubmit={verifyResetCode} className="mt-6 space-y-3">
                <div>
                  <label className="text-xs font-medium text-muted-foreground">Code from email</label>
                  <input value={code} onChange={(e) => setCode(e.target.value.replace(/\D/g, "").slice(0, 8))}
                    inputMode="numeric" placeholder="123456" required
                    className="mt-1 w-full rounded-lg border border-border bg-input px-3 py-2 text-center text-lg tracking-[0.4em] outline-none focus:border-primary" />
                </div>
                <div>
                ...
                <button type="button" onClick={sendResetCode as unknown as () => void} className="w-full text-xs text-muted-foreground hover:text-foreground">
                  Didn't get it? Send again
                </button>
              </form>
            )}
            <p className="mt-4 text-center text-sm text-muted-foreground">
              <button onClick={() => { setMode("signin"); setResetStep("email"); }} className="font-medium text-primary hover:underline">
                Back to sign in
              </button>
            </p>
          </>
            ...
            <form onSubmit={submit} className="space-y-3">
              ...
              <div>
                <div className="flex items-center justify-between">
                  ...
                  {mode === "signin" && (
                    <button type="button" onClick={() => { setMode("forgot"); setResetStep("email"); }} className="text-xs font-medium text-primary hover:underline">
                      Forgot password?
                    </button>
                  )}
                </div>
